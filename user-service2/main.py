        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if cached_data:
                    ...
                    print(
                        f"[Redis Cache] HIT for key: {cache_key} - Response time: {cache_time*1000:.2f}ms"
                    )
            ...
            print(
                f"[Redis Cache] Function execution time: {function_time*1000:.2f}ms, Total processing time: {total_time*1000:.2f}ms"
            )
...
def invalidate_cache(pattern="*"):
    try:
        ...
        if keys:
            ...
            print(
                f"[Redis Cache] Invalidated {len(keys)} keys matching pattern: {pattern}"
            )
        else:
...
async def metrics_middleware(request, call_next):
    ...
    if not request_path.startswith("/metrics"):
        ...
        if request_method == "GET" and request_path == "/users/profile":
            print(
                f"[Performance] {request_method} {request_path} - Total response time: {duration*1000:.2f}ms, Status: {status_code}"
            )
...
def get_db_connection():
    ...
    )  # Default olarak yerel postgres container
    ...
    print(
        f"Connecting to PostgreSQL: host={host}, port={port}, database={database}, user={user}"
    )
    ...
    if supabase_client is None and SUPABASE_URL and SUPABASE_KEY:
        try:
            ...
            try:
                ...
                print(
                    f"Supabase connection test successful. Count: {test_response.count if hasattr(test_response, 'count') else 'unknown'}"
                )
...
async def create_user_in_supabase(
    ...
    try:
        ...
        try:
            ...
            print(
                f"'users' table exists with count: {table_query.count if hasattr(table_query, 'count') else 'unknown'}"
            )
            ...
            try:
                ...
                print(
                    "Supabase API doesn't support table creation. Please create the table manually in Supabase dashboard."
                )
                return None
...
def init_db():
    ...
    try:
        ...
        if supabase_client:
            ...
            print(
                "WARNING: Supabase client initialization failed or credentials missing"
            )
    except Exception as e:
...
async def setup_rabbitmq():
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            # Declare queues
            user_queue = await rabbitmq_channel.declare_queue(
                "user_queue", durable=True
            )
            load_queue = await rabbitmq_channel.declare_queue(
                "load_queue", durable=True
            )
...
async def send_message_to_rabbitmq(message, routing_key: str):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if (
                rabbitmq_exchange
                and rabbitmq_connection
                and rabbitmq_connection.is_closed is False
            ):
                await send_message(routing_key, message)
                print(
                    f" [x] Event sent: {message.event_type if hasattr(message, 'event_type') else ''}"
                )
                return
        ...
        return JSONResponse(
            content={
                ...
                "warning": (
                    None
                    if supabase_saved
                    else "User created in Firebase but not saved to Supabase database"
                ),
            },
            status_code=201,
        )
        ...
        return JSONResponse(
            content={
                ...
                "warning": (
                    ...
                    else "User exists in Firebase but not in Supabase, using fallback data"
                ),
            }
        )
    except Exception as e:
...
async def google_signup(request: dict):
    ...
    try:
        ...
        if existing_user:
            ...
            if len(existing_user.get("id", "")) == 36 and "-" in existing_user.get(
                "id", ""
            ):
                return JSONResponse(
                    content={
...
async def get_profile(authorization: str = Header(None)):
    ...
    try:
        ...
        if (
            firebase_data
            and hasattr(firebase_data, "provider_data")
            and firebase_data.provider_data
        ):
        ...
        if not user_data and firebase_data:
            print(
                f"User not found in Supabase, using Firebase data: {firebase_data.email}"
            )
            supabase_available = False
            ...
            try:
                if (
                    hasattr(firebase_data, "provider_data")
                    and firebase_data.provider_data
                ):
                    providers = [p.provider_id for p in firebase_data.provider_data]
            ...
            if supabase_result:
                print(
                    f"User successfully synced to Supabase during profile request: {firebase_data.email}"
                )
                user_data = supabase_result
                supabase_available = True
            else:
                print(
                    f"WARNING: Failed to sync user to Supabase during profile request: {firebase_data.email}"
                )
        ...
        if (
            user_data
            and user_data.get("role") is not None
            and user_data.get("role") != ""
        ):
            user_role = user_data.get("role")

        print(
            f"PROFILE - User role: '{user_role}', Raw value: '{user_data.get('role')}'"
        )
        ...
        return JSONResponse(
            content={
                ...
                "warning": (
                    None
                    if supabase_available
                    else "User profile from Firebase, not found in Supabase"
                ),
            }
        )
